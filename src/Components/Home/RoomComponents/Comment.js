...
const replies = [];
 ...
 class Comment extends Component {
    ...
    componentDidMount() {
        ...
        database.ref(`Comments/${shortID}`).once('value').then((snapshot) => {
            snapshot.forEach((childSnapShot) => {
                ...
                dummyComments.push({fullname:childSnapShot.val().fullname, comment:childSnapShot.val().comment, date: childSnapShot.val().date, photo:childSnapShot.val().photo })
            });
            ...
            database.ref(`CommentReplies/${shortID}`).once('value').then((snapshot) => { 
                snapshot.forEach((childSnapShot) => {
                    ...
                    // li.appendChild(document.createTextNode(childSnapShot.val().comment));
                    // li.style.marginLeft = '87px';
                    // ul.appendChild(li);
                    replies.push({
                        ...
                        commentIDReplyingTo:childSnapShot.val().commentIDReplyingTo,
    ...
    return(<div>
        ...
        <ul id={this.props.commentID} style={{marginLeft:100, display:'inline-block'}}>
            {/* {
